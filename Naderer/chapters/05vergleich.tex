\chapter{Vergleich der erweiterungsbasierten Entwicklung zur konventionellen Codeanpassung}
\label{cha:Vergleich}
...
GET:\code{ [Ok :=] Record.GET([Value] ,...)} \\
Liefert einen Record anhand seines Primärschlüssels.\\ \\
CALCFIELDS:\code{ [Ok :=] Record.CALCFIELDS(Field1, [Field2],...)} \\
...
VALIDATE: \code{Record.VALIDATE(Field [, NewValue])} \\
...
\subsubsection{Implementierung der Webservice-Anbindung in C/AL}
...
\subsubsection{Implementierung der Webservice-Anbindung in AL}
...
\end{itemize}
\paragraph{}
Mithilfe dieser vier Datentypen lassen sich beliebig komplexe und tief verschachtelte Json Dokumente erstellen und verarbeiten. Um einen Treuepunktposten an den Webdienst zu übertragen, sind jedoch keine komplexen Strukturen nötig. Die Verwendung von JsonObject reicht für diesen Anwendungsfall aus. Die Prozedur \textit{SendRequests()}, siehe Programm \ref{prog:wsHelper},  empfängt eine vorgefilterte Record-Variable von Treuepunktposten, und erstellt für jeden Posten ein neues JSON-Dokument \textit{EntryObject}, welches in weiterer Folge mithilfe der Hilfsprozedur \textit{DoPostRequest()}, siehe Programm \ref{prog:.netWS}, an den Webdienst übermittelt wird.
